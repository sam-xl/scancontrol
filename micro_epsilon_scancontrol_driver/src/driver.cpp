  ...
  : Node("scancontrol_driver")
{
  ...
  */
  ...
  if (config_.resolution > 0)
  {
    ...
    {
      RCLCPP_WARN_STREAM(logger, "Requested resolution of " << std::to_string(config_.resolution)
...
int ScanControlDriver::SetPartialProfile(int& resolution)
{
  ...
  RCLCPP_INFO_STREAM(logger, "Profile is losing " << std::to_string(lost_values)
...
int ScanControlDriver::SetDuration(unsigned int setting_id, unsigned int value)
{
  ...
  int ret_code = SetFeature(setting_id, encoded_value);
...
{
  int ret_code = GetFeature(setting_id, value);
...
{
  RCLCPP_FATAL(rclcpp::get_logger("scanCONTROL API"), "Conrol of scanCONTROL device lost!");
  rclcpp::shutdown();
}
