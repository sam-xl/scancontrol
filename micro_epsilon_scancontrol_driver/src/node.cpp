...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    RCLCPP_INFO(driver->logger, "Driver started");
  ...
  {
    RCLCPP_FATAL_STREAM(rclcpp::get_logger("micro_epsilon_scancontrol_driver"), "Unable to create driver node. Error: " << error.what());
    rclcpp::shutdown();
    return 0;
  }
}
